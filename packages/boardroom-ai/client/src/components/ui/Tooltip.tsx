...
interface TooltipProps {
   ...
   * wrapping content (max-w 16rem by default).
   */
  content: ReactNode;
  children: ReactNode;
  className?: string;
  ...
  delay?: number;
  /**
   * Override max-width when content is rich. Default 16rem.
   * Pass `null` to disable wrapping (single-line `whitespace-nowrap`).
   */
  maxWidth?: string | null;
}

/**
 * Lightweight tooltip primitive. Track F will replace with Radix; this version
 * adds hover + focus + ESC dismissal + ARIA so keyboard users get parity.
 *
 * Audit ref: UX-#2-edu — extended for Track D (rich content + ARIA).
 */
export function Tooltip({ content, children, className, delay = 300, maxWidth }: TooltipProps) {
  ...
  const widthStyle =
    maxWidth === null ? undefined : { maxWidth: maxWidth ?? (isRich ? '16rem' : undefined) };
