# BoardRoom AI + OmniMind Platform — Codex Instructions
...
Codex (Opus) is the sole build agent. Codex owns ALL packages: shared/, omnimind-api/, boardroom-ai/.
...
8. **No other LLM providers in v1.** Anthropic Codex only (Sonnet 4.6 + Haiku 4.5). See ADR-002.
...
| LLM | Anthropic SDK (Codex Sonnet 4.6 + Haiku 4.5) |
...
| `ANTHROPIC_API_KEY` | ✅ | ✅ | Codex Sonnet + Haiku |
...
- **After implementation:** Do a review pass — check against contracts, AGENTS.md rules, and shared types. Report issues, don't silently fix.
...
Post-implementation review:
  "Review the changes I just made against the contracts and AGENTS.md rules. Don't modify files, just report issues."
```
...
| 002 | Codex-only, no multi-model routing | 5,000+ paying users |
...
You are assisting Josh Laughter with BoardRoom AI + OmniMind Platform — an executive decision intelligence suite for solo founders, indie hackers, and consultants.
Product
BoardRoom gives users a personal board of 7 AI advisors (Optimist, Critic, Alternate, Technician, Questionnaire, Doer, CEO) that analyze decisions through distinct lenses, remember everything via a persistent cognitive memory layer, and synthesize actionable guidance. The CEO persona runs last, sees all outputs, and delivers a final synthesis.
Users choose a mode — decide (all 7), stress-test (Critic + Questionnaire + Alternate emphasis), plan (Doer + Technician emphasis), or brainstorm (Optimist + Alternate emphasis) — and the system dispatches the appropriate persona subset.
Architecture (Two Services, One Monorepo)
Browser → BoardRoom AI (React 19 + Express, port 3001)
              ↓ HTTP (x-api-key, timing-safe compare)
         OmniMind API (Express + Prisma 6.3, port 3333)
              ↓ Prisma Client
         PostgreSQL 16 (pgvector + pg_trgm + tsvector)
BoardRoom AI owns UX, persona dispatch, SSE streaming, and the custom ~200-line agent runtime. No LangChain, no CrewAI, no LangGraph — custom runtime only (ADR-001).
OmniMind API owns ALL persistent data. Memory validation pipeline, hybrid retrieval (vector + FTS + trigram), background jobs (node-cron). BoardRoom never touches the database directly — all data flows through OmniMind's REST API via omnimind-client.ts.
Shared package (@boardroom/shared) contains TypeScript interfaces, Zod schemas, constants, and pure utilities. Zero business logic.
Stack

Language: TypeScript everywhere (ES2022, strict, commonjs)
Build: Turborepo monorepo, pnpm 10.32.1
Frontend: React 19 + Tailwind + CVA + Framer Motion, Zustand 5.0 for state
APIs: Express 4.21 (both services)
Database: PostgreSQL 16 + Prisma 6.3, pgvector (1536-dim), pg_trgm, tsvector
LLM: Anthropic Claude only — Sonnet 4.6 (heavy reasoning) + Haiku 4.5 (cheap roles). No other providers in v1 (ADR-002).
Embeddings: OpenAI text-embedding-3-small (1536-dim), stored as vector(1536) in Postgres
Auth: JWT httpOnly cookies (BoardRoom → browser), API key + timing-safe compare (BoardRoom → OmniMind)
Background: node-cron (no Redis, no BullMQ) — ADR-009
Tools: Native Anthropic SDK tool_use content blocks, NOT MCP (ADR-008). Tools are plain TS functions.
Testing: Vitest (110+ tests), custom eval runners (retrieval, personas, e2e)
Deploy: Railway (auto-deploy on push to main), Docker multi-stage builds

Database (32 Prisma Models)
Key model groups: User/Team/TeamMember | Room/Session/TranscriptEntry/AdvisorMessage | MemoryEntry (with vector embedding) | Decision/DecisionAssumption | Commitment | Person/Goal/Project/Task | join tables (MemoryEntityLink, GoalProjectLink, etc.) | UserProfile/ContextCapsule | ThinkingPattern/ContradictionAlert/WeeklyMemo/OutcomeReviewNudge | CustomPersona | Subscription/OAuthToken.
Soft deletes (deletedAt DateTime?) on: Room, Session, MemoryEntry, Decision, Commitment, Person, Goal, Project, Task. All queries must filter WHERE deletedAt IS NULL.
Memory & Retrieval
Every memory write goes through a validation pipeline: Zod schema → temporal validation → budget enforcement. No raw Prisma inserts.
Hybrid retrieval: semantic search (pgvector cosine similarity) + fulltext search (tsvector) + trigram fuzzy matching (pg_trgm) + structured SQL filters. Results combined and limited to 7-10 context items per persona call by context-packager.ts.
Key Conventions

All types in packages/shared/src/types/ using interface (not type aliases), TypeScript enum (matching Prisma), camelCase fields, string IDs, Date timestamps
Zod schemas in packages/shared/src/validation/ must structurally match companion types — change one, change both
All LLM outputs validated with Zod before reaching users
Persona prompts live in docs/prompts/*.system.md — edit prompts in markdown, not in TypeScript
Never delete working code to "simplify" — add alongside, deprecate later

Deployment
Both services live on Railway, auto-deploy on push to main.

BoardRoom: boardroom-ai-production-1092.up.railway.app
OmniMind: omnimind-api-production.up.railway.app

Docker builds require strict ordering: shared builds first, rm -f tsconfig.tsbuildinfo before tsc, Prisma CLI pinned to 6.19.3 (7.x breaks schema), pnpm deploy --legacy --prod for real node_modules.
Express middleware ordering in BoardRoom is load-bearing: global middleware → API prefix rewriting → static serving + SPA fallback → public routes → auth wall → protected routes → error handler. Do not rearrange.
Phase Status

Phase 0 (Foundation): ✅ Complete — Prisma schema, CRUD, validation, retrieval, Docker, agent runtime
Phase 1 (Multi-Persona): ✅ Complete — 7 personas, dispatch, streaming, synthesis, extraction, eval
Phase 2 (Cortex Intelligence): Spec'd — pattern detection, weekly memos, contradiction alerts, simulation
Phase 3 (Integrations): Spec'd — Google Calendar, Gmail, Stripe billing, custom personas
Phase 4+ (Collaboration): Future — multi-user, scaling

Architectural Decisions (13 ADRs — Don't Re-Litigate)
ADRDecisionRevisit Threshold001Custom agent runtime, no frameworksSeries A002Claude-only, no multi-model5,000+ paying users003PostgreSQL + pgvector, no separate vector DB10M+ vectors004No knowledge graph in v1500+ memories/user008Native tool_use, no MCPv2 migration009node-cron, no Redis500+ users or jobs >30s011OpenAI embeddings (1536-dim)Cost/quality review
Project Documentation

docs/MASTER-FRAMEWORK.md — 80kb full product + architecture spec
docs/DECISIONS.md — all 13 ADRs with rationale
docs/PROJECT-BRIEF.md — 1-page session-start context
docs/CURRENT-STATE.md — what's deployed, what's next
docs/ARCHITECTURE-QUICK-REF.md — compressed architecture map, file tree, data flows
docs/FRAGILE-ZONES.md — what breaks easily and why
docs/DEPLOYMENT-RUNBOOK.md — Railway config, env vars, common issues
docs/contracts/ — API contracts between services
docs/prompts/ — 20 persona system prompts (7 core + 6 specialized + 7 cortex)
docs/tasks/ — phase-based task specs with master index

Known Limitations

No CI/CD gate — manual typecheck/test before push
In-memory rate limiting — resets on restart
Public domain for service-to-service calls (should be Railway private networking)
prisma db push instead of proper migration history
Subscription middleware fails open when OmniMind is unreachable
